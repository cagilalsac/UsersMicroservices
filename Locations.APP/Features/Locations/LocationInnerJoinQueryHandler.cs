...
namespace Locations.APP.Features.Locations
{
    ...
    public class LocationInnerJoinQueryRequest : Request, IRequest<IQueryable<LocationInnerJoinQueryResponse>>, IPageRequest, IOrderRequest
    {
        ...
        /// <summary>
        /// Gets or sets a value indicating whether the ordering is ascending for ordering.
        /// </summary>
    ...
    public class LocationInnerJoinQueryHandler : Service<Country>, IRequestHandler<LocationInnerJoinQueryRequest, IQueryable<LocationInnerJoinQueryResponse>>
    {
        ...
        public Task<IQueryable<LocationInnerJoinQueryResponse>> Handle(LocationInnerJoinQueryRequest request, CancellationToken cancellationToken)
        {
            ...
            // Project the ordered ascending by country name then ordered ascending by city name result into a LocationInnerJoinQueryResponse DTO (Model).
            ...
            {
                // Filter results to include only those where the country name contains the provided value (case-insensitive).
                innerJoinQuery = innerJoinQuery.Where(location => location.CountryName.ToUpper().Contains(request.CountryName.ToUpper().Trim()));
            }

            // Apply filtering by city name if provided in the request.
            if (!string.IsNullOrWhiteSpace(request.CityName))
            {
                // Filter results to include only those where the city name contains the provided value (case-insensitive).
                innerJoinQuery = innerJoinQuery.Where(location => location.CityName.ToUpper().Contains(request.CityName.ToUpper().Trim()));
            }
